...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        x_pad = torch.nn.functional.pad(x,(pad, pad, pad, pad)) # (padding_left,padding_right, \text{padding\_top}, \text{padding\_bottom})padding_top,padding_bottom)
        out = torch.zeros(N, F, int(Hp), int(Wp), device = x.device, dtype = x.dtype)
        for n in range(N):
          for f in range(F):
            for h in range(Hp):
              for p in range(Wp):
                hp = h * stride
                wp = p * stride
                conv = x_pad[n, :, hp : hp + HH, wp : wp + WW] * w[f]
                out[n, f, h, p] = conv.sum() + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        dx_pad = torch.zeros_like(x_pad, dtype=x.dtype, device=dout.device)
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros([N, C, Hp, Wp], dtype=x.dtype, device=x.device)
        for i in range(Hp):
            for j in range(Wp):
          ...
          for c in range(C):
            for i in range(H_out):
              for j in range(W_out):
                ...
                dmask = torch.zeros(pool_height * pool_width, dtype = x.dtype, device = x.device)
                dmask[torch.argmax(block)] = 1
                dx[n, c, i * stride : i * stride + pool_height, j * stride : j * stride + pool_width] +=  torch.mul(dmask.view(pool_height,pool_width), dout[n, c, i, j])
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        newdims = num_filters * H // 2 * W // 2
        self.params["W2"] = torch.normal(0., weight_scale, [newdims, hidden_dim], dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        x1,conv_cache=Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        x2,cache1=Linear_ReLU.forward(x1, W2, b2)
        scores,cache2=Linear.forward(x2, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        loss += self.reg * (self.params["W1"] ** 2).sum() + self.reg * (self.params["W2"] ** 2).sum() + self.reg * (self.params["W3"] ** 2).sum()

        dx, grads["W3"], grads["b3"] = Linear.backward(grad1, cache2)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(1,self.num_layers + 1):
          if(self.batchnorm and i != self.num_layers):
            self.params['gamma' + str(i)] = torch.ones(num_filters[i - 1], dtype = dtype, device = device)
            self.params['beta' + str(i)] = torch.zeros(num_filters[i - 1], dtype = dtype, device = device)
          if(weight_scale == 'kaiming'):
            if(i == self.num_layers):
              num_pool = len(max_pools)
              linear_dim = num_filters[-1] * (H // (2 ** num_pool))* (W // (2 ** num_pool))
              self.params['W' + str(i)] = kaiming_initializer(linear_dim, num_classes, relu = False, dtype = dtype, device = device)
              self.params['b'+ str(i)] = torch.zeros(num_classes, dtype = dtype, device = device)
            else:
              self.params['W' + str(i)] =  kaiming_initializer(C, num_filters[i - 1], K=3, relu = True, dtype = dtype, device = device)
              self.params['b'+ str(i)] = torch.zeros(num_filters[i - 1], dtype = dtype, device = device)
              C = num_filters[i-1]
          else:
            if(i == self.num_layers):
              num_pool = len(max_pools)
              linear_dim = num_filters[-1] * (H // (2 ** num_pool))* (W // (2 ** num_pool))
              self.params['W'+str(i)] = torch.normal(0, weight_scale, [linear_dim, num_classes], dtype = dtype, device = device)
              self.params['b'+str(i)] = torch.zeros(num_classes, dtype = dtype, device = device)
            else:
              self.params['W' + str(i)] =  weight_scale * torch.randn(num_filters[i - 1], C, 3, 3, dtype = dtype, device = device)
              self.params['b'+ str(i)] = torch.zeros(num_filters[i - 1], dtype = dtype, device = device)
              C = num_filters[i - 1]    
        ################################################################
        #                      END OF YOUR CODE                        #
    ...
    def loss(self, X, y=None):
        ...
        grad1, dw, db = Linear.backward(grad1, caches.pop())
        ...
        for i in range(0, self.num_layers-1):
          if self.num_layers- 2 - i in self.max_pools:
            if self.batchnorm:
              grad1, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(grad1, caches.pop())
              ...
              grad1, dw, db = Conv_ReLU_Pool.backward(grad1, caches.pop())
              grads['W' + str(self.num_layers -1 - i)] = dw + 2 * self.reg * self.params['W' + str(self.num_layers -1 - i)]
              grads['b' + str(self.num_layers -1 - i)] = db
          else:
            if self.batchnorm:
              grad1, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(grad1, caches.pop())
              grads['W' + str(self.num_layers -1 - i)] = dw + 2 * self.reg * self.params['W' + str(self.num_layers -1 - i)]
              grads['b' + str(self.num_layers -1 - i)] = db
              grads['gamma' + str(self.num_layers -1 - i)] = dgamma
              grads['beta' + str(self.num_layers -1 - i)] = dbeta
            else:
              grad1, dw, db = Conv_ReLU.backward(grad1, caches.pop())
              grads['W' + str(self.num_layers -1 - i)] = dw + 2 * self.reg * self.params['W' + str(self.num_layers -1 - i)]
              grads['b' + str(self.num_layers -1 - i)] = db
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight_scale = torch.sqrt(torch.tensor(gain / Din, dtype=dtype, device=device))
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            x_mu = x - sample_mean
            sq_mu = x_mu ** 2
            sample_var = torch.sum(sq_mu, dim = 0) / N
            sqrt_var = torch.sqrt(sample_var + eps)
            invert_var = 1 / sqrt_var
            x_norm = x_mu * invert_var
            out = gamma * x_norm + beta
            cache = (x_norm, gamma, x_mu, invert_var, sqrt_var, sample_var, eps, x, mode)
            ...
            out = gamma * y + beta
            cache = (x1, gamma, x, invert_var, sqrt_var, running_var, eps, x_norm, mode)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, D = dout.shape
        x_norm, gamma, x_mu, invert_var, sqrt_var, sample_var, eps, x, mode = cache
        if mode == 'train':
          dbeta = torch.sum(dout, dim = 0)
          dgamma = torch.sum(dout * x_norm, dim = 0)
          dx_norm = dout * gamma
          divar = torch.sum(dx_norm * x_mu, dim = 0)
          dx_mu = dx_norm * invert_var
          dsqart_var = -1 / (sqrt_var ** 2) * divar
          dvar = 0.5 * 1. / torch.sqrt(sample_var + eps) * dsqart_var
          dsq = (1 / N) * dvar 
          dx1 = dx_mu + 2 * x_mu * dsq
          ...
          dbeta = torch.sum(dout, dim = 0)
          dgamma = torch.sum(dout * x, dim = 0)
          dxnorm = dout * gamma
          divar = torch.sum(dxnorm * x, dim = 0)
          dx = dxnorm * invert_var
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        dx = invert_var * (dout * gamma - torch.mean(dout * gamma, dim=0) - x_norm * torch.mean(dout * gamma * x_norm, dim=0))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        x = x.permute(0, 2, 3, 1).reshape(-1, C).contiguous()
