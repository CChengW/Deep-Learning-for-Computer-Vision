...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        db = dout.sum(dim = 0)
        dx = torch.mm(dout, w.t()).reshape(x.size())
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        hidden, fc1_cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, fc2_cache = Linear.forward(hidden, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        ###################################################################
        # Replace "pass" statement with your code
        loss1, upstream_grad = softmax_loss(scores, y)
        loss = loss1 + self.reg * (torch.sum(self.params['W1'] ** 2)+torch.sum(self.params['W2'] ** 2))
        dx2, dw2, db2 = Linear.backward(upstream_grad, fc2_cache)
        grads['W2'] = dw2 + 2 * self.reg * self.params['W2']
        grads['b2'] = db2
        dx1, dw1, db1 = Linear_ReLU.backward(dx2, fc1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for idx, hidden_dim in enumerate(hidden_dims):
          self.params['W{}'.format(idx+1)] = torch.normal(0, weight_scale, [in_dim, hidden_dim], dtype=dtype, device=device)
          self.params['b{}'.format(idx+1)] = torch.zeros(hidden_dim, dtype=dtype, device=device)
          in_dim = hidden_dim
        # last layer
        self.params['W{}'.format(self.num_layers)] = torch.normal(0, weight_scale, [hidden_dims[-1], num_classes], dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
          if (i == self.num_layers):
            scores, cache['{}'.format(self.num_layers)] = Linear.forward(scores, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
          else:
            scores, cache['{}'.format(i)] = Linear_ReLU.forward(scores, self.params['W{}'.format(i)], self.params['b{}'.format(i)])
            if self.use_dropout:
              scores, dropout_cache['{}'.format(i)] = Dropout.forward(scores, self.dropout_param)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(self.num_layers, 0, -1):
            if (i == self.num_layers):
                upstream_grad, grads['W{}'.format(self.num_layers)], grads['b{}'.format(self.num_layers)] =Linear.backward(upstream_grad, cache['{}'.format(self.num_layers)]) 
            else: 
              if self.use_dropout:
                  upstream_grad = Dropout.backward(upstream_grad, dropout_cache['{}'.format(i)])
              upstream_grad, grads['W{}'.format(i)], grads['b{}'.format(i)] =Linear_ReLU.backward(upstream_grad, cache['{}'.format(i)])  
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, num_epochs = 40, lr_decay = 0.97, optim_config = {'learning_rate':1e-1}, device = device, print_every=100)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def adam(w, dw, config=None):
    ...
    next_w = w - config["learning_rate"] * moment1_unbias / (moment2_unbias.sqrt() + config["epsilon"])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
