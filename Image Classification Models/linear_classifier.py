...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    grad_margin = margin
    grad_margin[margin > 0] = 1
    row_sum = torch.sum(margin, dim = 1)
    grad_margin[torch.arange(num_train), y] = -row_sum
    dW += (X.T).mm(margin)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    #torch.randint(low=0, high, size, *, generator=None, out=None, dtype=None, layout=torch.strided, device=None, requires_grad=False) → Tensor返回一个张量，该张量填充在 low （包含）和 high （不含）之间均匀生成的随机整数。
...
def svm_get_search_params():
    ...
    regularization_strengths = [8e-3, 7e-2, 2e-2, 4e-1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    loss_history = cls.train(X_train, y_train, lr, reg, num_iters=num_iters, batch_size=200,verbose=True)
    y_train_pred = cls.predict(X_train)
    y_val_pred = cls.predict(X_val)
    train_acc = (y_train_pred == y_train).double().mean().item()*100
    val_acc = (y_val == y_val_pred).double().mean().item()*100
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = torch.mm(X, W)
    z = scores - (torch.max(scores, dim=1, keepdim = True)[0])
    scores_softmax = torch.exp(z) / torch.sum(torch.exp(z), dim = 1, keepdim = True)
    loss += torch.sum(-torch.log(scores_softmax[torch.arange(num_train), y]))
    scores_softmax[range(num_train), y] -= 1
    dW += X.t().mm(scores_softmax)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2, 2e-2, 3e-2, 4e-3, 5e-3]
    regularization_strengths = [8e-3, 7e-4, 2e-2, 4e-1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
