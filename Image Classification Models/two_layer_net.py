...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores_softmax = torch.exp(scores) / torch.sum(torch.exp(scores), dim = 1, keepdim = True)
    loss = torch.sum(-torch.log(scores_softmax[range(N), y]))
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores_softmax[torch.arange(N), y] -= 1
    dri_softmax = scores_softmax
    dri_softmax /= N

    grads['W2'] = h1.t().mm(dri_softmax)
    grads['W2'] += 2 * reg * W2
    grads['b2'] = torch.sum(dri_softmax, dim=0)

    dri_h1 = dri_softmax.mm(W2.t())
    dri_h1[h1 <= 0] = 0
    grads['W1'] = X.t().mm(dri_h1)
    grads['W1'] += 2 * reg * W1
    grads['b1'] = torch.sum(dri_h1, dim=0)    
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2, 0.1, 0.8]
    hidden_sizes = [256, 128, 64]
    regularization_strengths = [1e-3, 1e-4, 1e-5]
    learning_rate_decays = [1.0, 0.95, 0.8]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    x_train,x_val=data_dict["X_train"],data_dict["X_val"]
    y_train,y_val=data_dict["y_train"],data_dict["y_val"]
    for lr in learning_rates:
        for hs in hidden_sizes:
            for rs in regularization_strengths:
                for lrd in learning_rate_decays:
                    net = TwoLayerNet(3 * 32 * 32, hs, 10, device = x_train.device, dtype = x_train.dtype)
                    stats=net.train(x_train, y_train, x_val, y_val,num_iters = 3000, batch_size = 1000, learning_rate = lr, learning_rate_decay = lrd,reg = rs, verbose = False)
