...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i][j] = torch.sum((x_test[j] - x_train[i]) ** 2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.view(num_train, -1)
    x_test = x_test.view(num_test, -1)
    m1 = torch.sum(x_train ** 2, dim = 1).reshape(num_train, 1)
    m2 = torch.sum(x_test ** 2, dim = 1)
    x_test_trans = torch.t(x_test)
    m3 = torch.mm(x_train, x_test_trans)
    dists = m1 + m2 - 2 * m3
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        _, idx = torch.topk(dists[:, i], k = k, dim = 0, largest = False)
        temp = y_train[idx]
        val, _ = torch.mode(temp, 0)
        y_pred[i] = val
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_train_data = x_train_folds[:i] + x_train_folds[i+1: ]
            y_train_data = y_train_folds[:i] + y_train_folds[i+1: ]
            x_test_data = x_train_folds[i]
            y_test_data = y_train_folds[i]
            x_train = torch.cat(x_train_data)
            y_train = torch.cat(y_train_data)
            knn_classifier = KnnClassifier(x_train, y_train)
            accuracies = knn_classifier.check_accuracy(x_test_data, y_test_data, k)
            k_to_accuracies[k].append(accuracies)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracies in sorted(k_to_accuracies.items()):
        ...
        if acc_mean > smallest or (acc_mean == smallest and k < best_k):
            smallest = acc_mean
            best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
