...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for stem_channel in stem_channels:
            conv_cls = nn.Conv2d(in_channels, stem_channel, kernel_size = 3, padding = 1)
            nn.init.normal_(conv_cls.weight, 0, 0.01) 
            stem_cls.append(conv_cls)
            stem_cls.append(nn.ReLU())

            conv_box = nn.Conv2d(in_channels, stem_channel, kernel_size = 3, padding = 1)
            nn.init.normal_(conv_box.weight, 0, 0.01)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for key, size in feats_per_fpn_level.items():
            class_logits[key] = self.pred_cls(self.stem_cls(size)) 
            batch_size = class_logits[key].shape[0]
            num_classes = class_logits[key].shape[1]
            class_logits[key] = class_logits[key].view(batch_size, num_classes, -1) 
            class_logits[key] = class_logits[key].permute(0, 2, 1) 
            
            stem_box_output = self.stem_box(size)
            boxreg_deltas[key] = self.pred_box(stem_box_output)
            boxreg_deltas[key] = boxreg_deltas[key].view(batch_size, 4, -1)
            boxreg_deltas[key] = boxreg_deltas[key].permute(0, 2, 1)

            centerness_logits[key] = self.pred_ctr(stem_box_output)
            centerness_logits[key] = centerness_logits[key].view(batch_size, 1, -1)
...
def fcos_get_deltas_from_locations(
    ...
    N, _ = locations.shape
    deltas = locations.new_ones([N, 4]).to(device = gt_boxes.device, dtype = gt_boxes.dtype)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    ##########################################################################
    LR_deltas = deltas[:, 0::2]
    TB_deltas = deltas[:, 1::2]
    minLR = torch.min(LR_deltas, dim=1).values
    minTB = torch.min( TB_deltas, dim=1).values
    maxLR = torch.max(LR_deltas, dim=1).values
    maxTB = torch.max( TB_deltas, dim=1).values
    centerness = torch.sqrt((minLR * minTB) / (maxLR * maxTB))
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        features_shape = {"p3" : fpn_features["p3"].shape, 
                          "p4" : fpn_features["p4"].shape, 
                          "p5" : fpn_features["p5"].shape}
        locations_per_fpn_level = get_fpn_location_coords(features_shape, self.backbone.fpn_strides, device = images.device)
        # Replace "pass" statement with your code
        ...
        for batch in range(gt_boxes.shape[0]):
            ...
            matched_boxes_per_image = fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, gt_boxes[batch])
            for level_name, matched_boxes in matched_boxes_per_image.items():
        ...
        for batch in range(gt_boxes.shape[0]):
            ...
            for level_name, locations in locations_per_fpn_level.items():
                matched_gt_deltas_batch[level_name] = fcos_get_deltas_from_locations(locations, matched_gt_boxes[batch][level_name], self.backbone.fpn_strides[level_name])
        ...
        matched_gt_ctr = torch.zeros(a * b).to(device=matched_gt_boxes.device, dtype=matched_gt_boxes.dtype)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            level_pred_scores, idx = level_pred_scores.max(dim=1)

            # Step 2:
            # Replace "pass" statement with your code
            keep = level_pred_scores > test_score_thresh
            level_pred_classes =idx[ keep]
            ...
            non_background = (level_deltas.sum(dim=1) != -4)
            level_pred_boxes = level_pred_boxes[non_background]
            level_pred_classes = level_pred_classes[non_background]
            level_pred_scores = level_pred_scores[non_background]

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            _, _, H, W = images.shape
            level_pred_boxes[:, 0] = torch.clip(level_pred_boxes[:, 0], min = 0)
            level_pred_boxes[:, 1] = torch.clip(level_pred_boxes[:, 1], min = 0)
            level_pred_boxes[:, 2] = torch.clip(level_pred_boxes[:, 2], max = W)
            level_pred_boxes[:, 3] = torch.clip(level_pred_boxes[:, 3], max = H)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
