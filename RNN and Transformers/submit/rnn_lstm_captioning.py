...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    lasth = prev_h.clone()
    temp =lasth @ Wh + x @ Wx + b
    next_h = torch.tanh(temp)
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x, prev_h, Wh, Wx, temp = cache

    #compute gradient of tanhx: 1 - tanh^2(x)
    dl = (1 - temp * temp) * dnext_h
    dx = dl @ Wx.T
    dprev_h = dl @ Wh.T
    dWx = (x.T) @ dl
    dWh = (prev_h.T) @ dl
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    cache = []
    X = x.permute(1, 0, 2) #shape(T, N, D)
    h = torch.zeros(T, N, H).to(x)

    for i in range(T):
        h[i], ca = rnn_step_forward(X[i], prev_h, Wx, Wh, b)
        prev_h = h[i]
        cache.append(ca)
    #now h shape: (T, N, H)
    h = h.permute(1, 0, 2) #shape (N, T, H)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    dx = torch.zeros(T, N, D).to(dh)
    dh0 = torch.zeros(N, H).to(dh) 
    dWx = torch.zeros(D, H).to(dh)
    dWh = torch.zeros(H, H).to(dh) 
    db = torch.zeros(H,).to(dh) 
    dhnew = dh.permute(1, 0, 2) #shape(T, N, H)

    for i in reversed(range(T)):
        temp = rnn_step_backward(dh0 + dhnew[i], cache[i])
        dx[i] = temp[0]
        dh0 = temp[1]
        dWx += temp[2]
        dWh += temp[3]
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.encoderimage = ImageEncoder()

        if cell_type in {"rnn", "lstm"}:
            self.feature_proj = nn.Sequential(nn.AvgPool2d(4, 4), nn.Flatten(), nn.Linear(input_dim, hidden_dim))
        elif cell_type is "attn":
            self.feature_proj = nn.Conv2d(input_dim, hidden_dim, 1, stride = 1, padding = 0)

        self.embeddingimage = WordEmbedding(vocab_size, wordvec_dim)

        if cell_type is "rnn":
            self.caption = RNN(wordvec_dim, hidden_dim)
        elif cell_type is "lstm":
            self.caption = LSTM(wordvec_dim, hidden_dim)
        elif cell_type is "attn":
            self.caption = AttentionLSTM(wordvec_dim, hidden_dim)
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        #(1) project the image feature to the initial hidden state $h0$
        N = images.shape[0]
        image_feature = self.encoderimage(images)
        h0 = self.feature_proj(image_feature)

        #(2) transform the words in captions_in from indices to vectors
        embedding_image = self.embeddingimage(captions_in)

        #(3) process the sequence of input word vectors
        hidden = self.caption(embedding_image, h0)

        #(4) compute scores over the vocabulary at every timestep
    ...
    def sample(self, images, max_length=15):
        ...
        #######################################################################
        imagefeature = self.encoderimage(images)
        prev_word = torch.full((N,), self._start)

        if self.cell_type is "rnn":
            prev_h = self.feature_proj(imagefeature) 
        elif self.cell_type is "lstm":
            prev_h = self.feature_proj(imagefeature) 
            prev_c = torch.zeros_like(prev_h) # initialize the first cell state to zeros
        elif self.cell_type is "attn":
            f = self.feature_proj(imagefeature) 
            prev_h = f.mean(dim=(2, 3))
            prev_c = prev_h
        
        for i in range(max_length):
            # (1) Embed the previous word
            embed = self.embeddingimage(prev_word)

            # (2) Make an RNN step
            if self.cell_type is "rnn":
                prev_h = self.caption.step_forward(embed, prev_h)
            elif self.cell_type is "lstm":
                prev_h, prev_c = self.caption.step_forward(embed, prev_h, prev_c)
            elif self.cell_type is "attn":
                atten, atten_weights = dot_product_attention(prev_h, f) 
                attn_weights_all[:, i] = atten_weights
                prev_h, prev_c = self.caption.step_forward(embed, prev_h, prev_c, atten)

            # (3) Apply the learned affine transformation to the next hidden state
            score = self.output_proj(prev_h)

            # (4) writing to the appropriate slot in the captions variable
            prev_word = torch.argmax(score, axis = 1)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        temp = prev_h @ self.Wh + x @ self.Wx + self.b
        inputg, forgetg, outputg, gateg = temp.chunk(4, 1)
        ig = torch.sigmoid(inputg)
        fg = torch.sigmoid(forgetg)
        og = torch.sigmoid(outputg)
        gg = torch.tanh(gateg)

        next_c = fg * prev_c + ig * gg
        next_h = og * torch.tanh(next_c)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        N, T, D = x.shape
        H = h0.shape[1]
        hn = torch.zeros(T, N, H).to(h0)
        X = x.permute(1, 0, 2) #size (T, N, D)
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    N, H, W, _ = A.shape
    At = torch.flatten(A, 2)
    Aw = torch.bmm(prev_h.reshape([N, 1, H]), At)
    Aw = Aw / math.sqrt(H)
    Aw = Aw.permute(0, 2, 1)
    Aw = torch.softmax(Aw, dim = 1)
    attn = torch.bmm(At, Aw).squeeze()
    attn_weights = Aw.reshape([N, W, W])
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N, H = prev_h.shape
        temp = x @ self.Wx + prev_h @ self.Wh + attn @ self.Wattn + self.b
        inputg, forgetg, outputg, gateg = temp.chunk(4, dim = 1)
