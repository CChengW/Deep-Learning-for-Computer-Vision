...
def scaled_dot_product_two_loop_single(
    ...
    K, M = query.shape
    k = key.shape[0]
    weight = query.new_zeros(K, k)
    for i, q in enumerate(query):
        for j, k in enumerate(key):
            weight[i,j] = q @ k / (M ** 0.5)
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(N):
        for j in range(K):
            q = query[i, j].unsqueeze(dim = -1) 
            weight = key[i] @ q / (M ** 0.5)
            weight = torch.softmax(weight, dim = 0).squeeze() 
            out[i, j] = weight @ value[i]
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        weight = torch.masked_fill(weight, mask, -1e9)
    # Replace "pass" statement with your code
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.trunc_normal_(self.q.weight, 0, 1, -c_q, c_q)
        nn.init.trunc_normal_(self.k.weight, 0, 1, -c_q, c_q)
        nn.init.trunc_normal_(self.v.weight, 0, 1, -c_v, c_v)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        for i in range(num_heads):
            self.single_attn.append(SelfAttention(dim_in, dim_out, dim_out))
        self.linear = nn.Linear(dim_out * num_heads, dim_in)
        c = 6 / (dim_out * num_heads + dim_in)
        c = c ** 0.5
        nn.init.trunc_normal_(self.linear.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        y = []
        for i in self.single_attn:
            y.append(i(query, key, value, mask))
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        var = ((x - mu) ** 2).mean(dim = -1).sqrt().unsqueeze(-1) + self.epsilon
        y = (x - mu) / var * self.gamma + self.beta
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        nn.init.trunc_normal_(self.MLP1.weight, -c, c)
        c = (6 / (hidden_dim_feedforward + inp_dim)) ** 0.5
        nn.init.trunc_normal_(self.MLP2.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multi_head_attn = MultiHeadAttention(num_heads, emb_dim, int(emb_dim / num_heads))
    ...
    def forward(self, x):
        ...
        temp = self.norm1(out1 + x)
        out2 = self.dropout(temp)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        temp = self.norm1(out1 + dec_inp)
        out2 = self.dropout(temp)
        out3 = self.attention_cross(out2, enc_inp, enc_inp)
        temp1 = self.norm2(out3 + out2)
        out4 = self.dropout(temp1)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = y.repeat(M, 1).permute([1, 0]).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.zeros(K, M)
    fl = torch.floor(torch.tensor(list(range(M))).unsqueeze(0) / M)
    p = torch.tensor(list(range(K))).unsqueeze(-1)  

    y[:, 0::2] = torch.sin(p / torch.pow(1e4, fl[0, 0::2]))
    y[:, 1::2] = torch.cos(p / torch.pow(1e4, fl[0, 1::2]))
    y = y.unsqueeze(0)  
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
