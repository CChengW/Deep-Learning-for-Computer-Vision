...
def compute_saliency_maps(X, y, model):
    ...
    N = X.shape[0]
    score = model(X)
    loss = torch.sum(score[torch.arange(N), y])
    loss.backward()
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        score = model(X_adv)
        val, idx = torch.max(score, axis = 1) 
        target_score = score[0, target_y].squeeze()
        print('Iteration %2d: target score %.3f, max score %.3f' % (i + 1, target_score.item(), val.item()))
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # Replace "pass" statement with your code
    score = model(img)
    loss = score[0, target_y].squeeze() - l2_reg * torch.square(torch.linalg.norm(img)) 
    loss.backward()
