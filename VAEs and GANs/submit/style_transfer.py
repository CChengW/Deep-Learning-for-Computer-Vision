...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.bmm(features, features.transpose(-2, -1))
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  guidance = masks.unsqueeze(dim = 2).repeat(1, 1, C, 1, 1)
  F = (features * guidance).view(N, R, C, H * W)
  F_trans = F.transpose(2, 3)
  guided_gram = F @ F_trans
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    for idx, i in enumerate(style_layers):
        current_guided_gram = guided_gram_matrix(feats[i], content_masks[i])
        loss += style_weights[idx] * torch.sum((current_guided_gram - style_targets[idx]) ** 2)
