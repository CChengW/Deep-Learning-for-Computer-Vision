...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        mu = self.mu_layer.forward(encode)
        logvar = self.logvar_layer.forward(encode)
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    ep = torch.randn(mu.shape,dtype = mu.dtype, device = mu.device)
    z = mu + ep * torch.exp(0.5 * logvar)
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    l1 = nn.functional.binary_cross_entropy(x_hat, x, reduction = 'sum')
    var = torch.exp(logvar)
    l2 = - 0.5 * (1 + logvar - mu ** 2 - var).sum(dim = 1)
    loss = l1 / x.shape[0] + l2.mean()
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
